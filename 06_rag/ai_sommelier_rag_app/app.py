...
해당파일(app.py)를 다시 실행한다.
"""

import logging # 오류를 파이썬 터미널에 출력하느 라이브러리
...
st.write("음식 이미지 URL을 입력하면 Wine Magazine 리뷰를 검색해 어울리는 와인을 추천한다.")
...
with st.form(key="image_url_form", clear_on_submit=False): # True일 시, 입력 제출 시에 소멸됨(ex.메세지 전송). False 는 남김.
...
if submitted:
    try:
    ...
    else: # image_url이 검증을 통과한 경우 이미지 출력
        st.image(
            image_url,
        ...
        try:
            with st.spinner("음식과 관련된 와인 리뷰를 분석중입니다..."): # 버퍼링 표시
